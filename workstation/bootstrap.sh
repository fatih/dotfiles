...
ExecStart=/usr/bin/docker run -h dev --net=host --rm -v /var/run/docker.sock:/var/run/docker.sock -v /mnt/code:/mnt/code -v /mnt/secrets:/mnt/secrets -v  --cap-add=SYS_PTRACE --security-opt seccomp=unconfined --privileged --name dev fatih/dev
